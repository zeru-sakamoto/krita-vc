...
export function RestoreModal({ archive, onClose }: { archive: string; onClose: () => void }) {
        ...
        <ul className="mt-3 flex flex-col gap-2">
          {results.map((r) => (
            <li key={r.dir} className="rounded-button bg-surface-3 px-2 py-1.5">
              <span className="block truncate text-body text-text">{r.name || r.dir}</span>
              {r.error ? (
                <span className="block text-caption text-danger">{r.error}</span>
              ) : (
                <>
                  <span className="block truncate text-caption text-text-muted">{r.path}</span>
  ...
  return (
    <>
      ...
      >
        <p className="break-all text-dense text-text-muted">
          {archive}
        ...
        <p className="mt-2 rounded-button bg-surface-3 px-2 py-1.5 text-dense text-text-muted">
          {storeRoot ? (
            <>
        ...
        {phase === "loading" && <p className="mt-3 text-dense text-text-muted">Reading backup…</p>}
                ...
                <li key={p.dir}>
                  <label className="flex cursor-pointer items-start gap-2 rounded-button px-2 py-1.5 transition-colors duration-(--dur-fast) ease-(--ease-out) hover:bg-state-hover">
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="block truncate text-body text-text">{p.relpath}</span>
                      <span className="block truncate text-caption text-text-muted">
                        {p.originalDirExists
                      ...
                      {p.occupied && (
                        <span className="mt-0.5 flex flex-wrap items-center gap-x-2 gap-y-1 text-caption text-warning-fg">
                          ...
                          {p.tracked && (
                            <button
                              type="button"
                              className="text-accent underline underline-offset-2 transition-[filter] duration-(--dur-fast) ease-(--ease-out) hover:brightness-110"
                              onClick={(e) => {
        ...
        {(needsFallback || fallbackDir) && (
          <div className="mt-3 flex items-center gap-2">
            <Button onClick={chooseFallback} disabled={busy}>
              <FolderOpen size={ICON.dense} />
              {fallbackDir ? "Change folder…" : "Choose a folder…"}
            </Button>
            <span className="min-w-0 flex-1 truncate text-dense text-text-muted">
